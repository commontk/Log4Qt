 ...
 *   - The \ref Log4Qt::AppenderSkeleton::doAppend() "doAppend()" member fuction will
 ...
 *     "handleIoErrors()" after all I/O oprations. Sub-classes should 
 *     reimplement the function to handle errors.
 * 
 ...
 *     ConfiguratorHelper provides a confiuration file watch and last error 
 ...
 * deletion during static deinitialization.
 * 
 ...
 * \ref Log4Qt::LogObject::release() "release()". Alternativly an auto pointer
 ...
 * "doAppend()".
 * 
 * The problem only occurs, if a loggger, appender, layout or filter log an 
 ...
 * static intialisation. The second stage takes place when the 
 ...
 * The creation is usally triggered by the request for a \ref Log4Qt::Logger 
 ...
 *     Log4Qt::Logger::logger("MyApplication")->info("Hallo World");
 * }
 * 
 * MyApplication::~MyApplication()
 * {
 ...
 * 
 * The package uses environment variables to control the intialisation 
 ...
 * Nice to have fetaures like for example Q_DECLARE_PRIVATE are not used. Only
 ...
 * 
 * - Reading / writing of bool or int is threadsafe, if declared volatile 
 ...
 *   in all sub-classes.
 *   - \ref Log4Qt::AppenderSkeleton "AppenderSkeleton"
 * - Being able to use singleton objects during static deinitilsation without 
...
namespace Log4Qt
{
     ...
     */
    #define LOG4QT_VERSION 0x000100
     ...
     *     \ref Log4Qt::LogManager::version() "LogManager::version()"
     */
    #define LOG4QT_VERSION_STR "0.1.0"
